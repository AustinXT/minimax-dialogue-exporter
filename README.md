...
- 📝 **Markdown格式**：导出为结构清晰的 Markdown 文件
...
1. 点击浏览器扩展图标，选择"添加新脚本"或"创建新脚本"
2. 将 `minimax-dialogue-exporter.user.js` 的内容复制粘贴到编辑器中
3. 保存脚本

或者：

1. 直接将 `.user.js` 文件拖拽到浏览器中
2. 在弹出的安装提示中点击"安装"

## 🎮 使用方法

1. 打开 MiniMax Agent 对话页面（如 `https://agent.minimaxi.com/share/...`）
2. ⚠️ **重要**：分享页面是演示动画，请**等待动画播放完成**后再操作
3. 页面右下角有两个按钮：
...
2. **元素类型识别**：
   ...
   - AI回复：`.message.received` 类 + `.matrix-markdown` 内容
